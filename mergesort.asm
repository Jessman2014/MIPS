...
# Jesse Dahir-Kanehl and Jonathan Loeffelholz
		...
		.data
amoPrompt:	.asciiz "Enter the number of elements in the array: "	#prompt for the number of elements
		.align 2						#align memory for following array of words
		
elePrompt:	.asciiz "Enter the array elements: "			#prompt for elements in the array
		.align 2						#align memory for following array of words
...
answer:		.asciiz "The elements sorted in ascending order are: "	#specifies what the answer is
		.align 2						#align memory for following array of words
# end of data segment
...
MAIN:  		la $a0, amoPrompt					#load prompt
		...
		add $s4, $zero, $s0					#store total number of elements for printing loop
		la $s1, arrY 						#put address of array start into $s1
		la $s2, space						#load address of space for use
		lw $s2, 0($s2)						#load value of space into register
		la $a0, elePrompt					#load prompt
		addi $v0, $zero, 4					#set up string print
		syscall							#print out prompt
...
END1:		add $a0, $zero, $s1					#put address of array start into first argument
		...
		la $a0, answer						#load answer string
		addi $v0, $zero, 4					#set up string print
		syscall							#print the string
		addi $s3, $zero, 0					#int i = 0
FOR2:		slt $t0, $s3, $s4					#create a for loop with i < elements in the array
		beq $t0, $zero, END2					#if i >= element number then go to END2 of loop
		sll $t0, $s3, 2						#multiply index by 4
		add $t0, $t0, $s1					#add offset to base address
		lw $a0, 0($t0)						#read int from array
...
MERGESORT1:	addi $sp, $sp, -20					#make space on stack for local variables
		...
		add $s1, $zero, $a3					# $s1 = e for later use
		slt $t0, $a2, $a3					# s < e
...
MERGE:		addi $sp, $sp, -20					#make space on stack for local variables
		sw $s0, 0($sp)						#store $s0
		sw $s1, 4($sp)						#store $s1
		sw $s2, 8($sp)						#store $s2
...
W4:		slt $t0, $s0, $s1					# e2 < i
		...
		sw $t0, 0($t1)						# Y[i] = A[i]
		addi $s1, $s1, 1					# i++	
E4:		lw $s0, 0($sp)						#restore $a0
		lw $s1, 4($sp)						#restore $a1
		lw $s2, 8($sp)						#restore $a2
		lw $s3, 12($sp)						#restore $a3
		addi $sp, $sp, 20					#pop the stack
		jr $ra							#return to caller
